...
class MondoController extends Controller
{
    ...
    public function redirectFromMondoAuth(Request $request)
    {
	    ...
	    if ($state === $request->session()->pull('mondo_auth_state'))
	    {
		    ...
		    curl_setopt($curl, CURLOPT_POSTFIELDS, 'grant_type=authorization_code&client_id='.urlencode($client_id).'&client_secret='.urlencode($client_secret).'&redirect_uri='.urlencode($redirect_uri).'&code='.urlencode($code));
    ...
    public function redirectToMondoRefresh(Request $request)
    {
	    ...
	    curl_setopt($curl, CURLOPT_POST, true);
	    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
	    curl_setopt($curl, CURLOPT_POSTFIELDS, 'grant_type=refresh_token&client_id='.urlencode($client_id).'&client_secret='.urlencode($client_secret).'&refresh_token='.urlencode($refresh_token));
	    $response = curl_exec($curl);
	    curl_close($curl);
